 ...
 * <p>
 * Implementations of this interface should keep the following invariant: Every user is always in exactly one session,
 * i.e. {@link #getSessionOf(User)} should always be unambiguously defined. Moreover, if a user leaves a session via
 * {@link #leaveSession(User)}, they should automatically be joined to a new, empty session.
 */
public interface GameSessionManager {
    ...
    /**
     * Retrieves the session a user is currently participating in. As a user is always in exactly one session, this will
